...
def create_app(test_config=None):
    ...
    def get_books():
        ...
        current_books = pagination_books(request, selection)
        if len(current_books) == 0:
            abort(404)
        return jsonify(
            {"success": True, "books": current_books, "total_books": len(selection),}
        )
    ...
    def create_book():
        ...
        try:
            ...
            abort(
                422
            )  # we are not able to process the request thought it is properly formatted
